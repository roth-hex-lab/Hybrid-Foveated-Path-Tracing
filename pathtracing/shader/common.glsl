...
vec3 sample_cone(vec3 axis, float max_angle_rad, vec2 random_sample) {
    float cos_theta_max = cos(max_angle_rad);
    float cos_theta = 1.0 - random_sample.x * (1.0 - cos_theta_max);
    float sin_theta = sqrt(1.0 - cos_theta * cos_theta);
    float phi = 2.0 * M_PI * random_sample.y;
    vec3 local_dir = vec3(cos(phi) * sin_theta, sin(phi) * sin_theta, cos_theta);
    return align(axis, local_dir);
}
...
// --- Integrated Albedo Calculation (Derived from direct_volume_rendering) ---
// This function calculates the integrated, unlit color (albedo) of the volume.

#define ALBEDO_STEPS 64 // TODO: Maybe this should be different (lower) than normal raymarching?

vec4 calculate_integrated_albedo(vec3 pos, vec3 dir, inout uint seed) {
    vec2 near_far;
    if (!intersect_box(pos, dir, vol_bb_min, vol_bb_max, near_far)) return vec4(0.0);

    // Convert to the volume's local index space for texture lookups.
    const vec3 ipos = vec3(vol_density_inv_transform * vec4(pos, 1));
    const vec3 idir = vec3(vol_density_inv_transform * vec4(dir, 0)); // non-normalized!
    
    const float dt = (near_far.y - near_far.x) / float(ALBEDO_STEPS);
    near_far.x += rng(seed) * dt; // jitter starting position

    vec3 accumulated_color = vec3(0.0);
    float Tr = 1.f;

    for (int i = 0; i < ALBEDO_STEPS; ++i) {
        float t = min(near_far.x + i * dt, near_far.y);
        const vec4 rgba = tf_lookup(lookup_density_trilinear(ipos + t * idir) * vol_inv_majorant);
        const float dtau = rgba.a * vol_majorant * dt;

        // Only blend if there's some opacity. TODO: Is this worth it?
        if (dtau > 0.001) { 
            accumulated_color += rgba.rgb * vol_albedo * dtau * Tr;
            Tr *= exp(-dtau);
        }

        if (Tr <= 0.01) { // We bail out way earlier than ray marching as we need to be fast here and it wont make a big perceptual difference
            break;
        }
    }

    return vec4(accumulated_color, 1.0 - Tr);
}
...
vec2 trace_path(vec3 pos, vec3 dir, inout uint seed, out vec4 L_out, mat4 current_VP, mat4 prev_VP, vec2 resolution, bool first_sample, bool calculate_motion) {
    // trace path
    vec3 L = vec3(0);
    ...
    bool motion_vector_calculated = false;
    ...
    while (sample_volume(pos, dir, t, throughput, L, seed)) {
        ...
        if (calculate_motion && first_sample && !motion_vector_calculated) {
            vec4 current_pos_clip = current_VP * vec4(pos, 1.0);
            vec4 prev_pos_clip = prev_VP * vec4(pos, 1.0);

            // Perspective divide
            vec2 current_pos_ndc = current_pos_clip.xy / current_pos_clip.w;
            vec2 prev_pos_ndc = prev_pos_clip.xy / prev_pos_clip.w;
            
            // Convert from NDC [-1, 1] to pixel coordinates [0, resolution]
            vec2 current_pos_pixels = (current_pos_ndc * 0.5 + 0.5) * resolution;
            vec2 prev_pos_pixels = (prev_pos_ndc * 0.5 + 0.5) * resolution;

            // The motion vector is the displacement in pixels
            // This is the direction OptiX expects: from previous to current frame
            motion_vec = current_pos_pixels - prev_pos_pixels;

            motion_vector_calculated = true;
        }
    ...
    L_out = vec4(L, clamp(n_paths, 0.f, 1.f));
    return motion_vec;
}

void trace_path_multi(vec3 pos, vec3 dir, inout uint seed, out vec4 L_out, 
                      bool generate_motionvec, mat4 current_VP, mat4 prev_VP, vec2 resolution, out vec4 MV_out,
                      bool generate_depth, out vec4 D_out) {
    ...
    while (sample_volume(pos, dir, t, throughput, L, seed)) {
        ...
        if (generate_motionvec) {
            ...
            // The motion vector is the displacement in pixels
            // This is the direction OptiX expects: from previous to current frame
            MV_out = vec4(current_pos_pixels - prev_pos_pixels, 0, 0);
        }

        if (generate_depth) {
            D_out = vec4(clamp(length(pos - cam_pos) * 0.25f, 0.0f, 1.0f), 0, 0, 0);
        }
...
vec4 trace_path_pointcloud(vec3 pos, vec3 dir, inout uint seed) {
    vec4 L;
    mat4 unused_mat4;
    vec4 unused_vec4;
    vec2 unused_vec2;
    trace_path_multi(pos, dir, seed, L, 
                     false, unused_mat4, unused_mat4, unused_vec2, unused_vec4, // No motion
                     false, unused_vec4);                                       // No depth
    return L;
}