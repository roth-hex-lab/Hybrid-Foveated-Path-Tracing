...
static int pc_num = 10000;
static int pc_sppx = 64;
...
void run_script(const std::string& path) {
    ...
    // Use a custom interpreter that doesn't auto-finalize
    static bool python_initialized = false;
    if (!python_initialized) {
        Py_Initialize();
        python_initialized = true;
        
        // Prevent automatic finalization by registering a no-op atexit handler
        std::atexit([]() {
            // Don't call Py_FinalizeEx() - let the OS clean up
            // This prevents the double-free errors
        });
    }
    ...
    try {
        ...
        try {
            ...
            error_msg = "Failed to get error message";
        }
        
        std::cerr << "Python error in script " << path << ":\n\n" << error_msg << std::endl;
        PyErr_Clear();
        std::exit(1);
...
void gui_callback(void) {
    ...
    if (ImGui::Begin("Stuff", 0, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoFocusOnAppearing)) {
        ...
        if (ImGui::SliderFloat("Camera FOV", &fov, 1.0f, 140.0f)) {
            current_camera()->fov_degree = fov;
            renderer->reset();
        }
        if (ImGui::SliderFloat("Camera Speed", &CAM_SPEED, 0.1f, 10.f)) {
            CameraImpl::default_camera_movement_speed = CAM_SPEED * 0.0001;
        }
        if (ImGui::Checkbox("Vsync", &use_vsync)) Context::set_swap_interval(use_vsync ? 1 : 0);
        ImGui::SameLine();
        if (ImGui::Checkbox("Denoising", &renderer->denoising_enabled)) {
            if (!renderer->denoising_enabled) {
                renderer->show_denoised = false;
            }
        }
        bool perspective = current_camera()->perspective;
        if (ImGui::Checkbox("Perspective", &perspective)) {
            std::cout << "Switching perspective to " << (current_camera()->perspective) << std::endl;
            current_camera()->perspective = perspective;
        ...
        ImGui::DragInt("PC Num", &pc_num, 10, 1, 1000000);
        ImGui::SameLine();
        ImGui::DragInt("SPPX", &pc_sppx, 1, 1, 1024);
        if (ImGui::Button("To Points3D")) renderer->to_pointcloud_gpu(pc_num, pc_sppx);
...
int main(int argc, char** argv) {
    ...
    CameraImpl::default_camera_movement_speed = CAM_SPEED * 0.0001; // Slow down some from default 0.005f speed

    // parse command line arguments
    parse_cmd(argc, argv);
    if (scriptmode) {
        renderer->reset();
        return 0; // We just want to bail after loading the script. Segfaults, but so does doing nothing. Fix later.
    }
    ...
    if (interactive) {
        ...
        for (int i = 0; i < renderer->volume->n_grid_frames(); ++i) {
            ...
            tonemap_shader->bind();
            renderer->color_texture->bind_image(0, GL_READ_WRITE, GL_RGBA32F);
            ...
            renderer->color_texture->unbind_image(0);
            ...
            renderer->color_texture->save_ldr(out_fn);
