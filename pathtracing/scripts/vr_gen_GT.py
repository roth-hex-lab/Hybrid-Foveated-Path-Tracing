...
from vr_gen_config_GT import general_settings, offline_renderer_config, datasets
...
def render_view_set(
    ...
    # Hacky shit for now
    collect_GT_poses = filename_prefix.startswith("GT")
    print("Collecting poses: ", collect_GT_poses)
    ...
    for i in range(n_views):
        ...
        while not is_view_valid and retry_count < max_retries:
            ...
            if collect_GT_poses or not GT_POSES:
                renderer.cam_pos = cam_pos 
                renderer.cam_dir = (cam_target - cam_pos).normalize()
            else:
                renderer.cam_pos = GT_POSES[i][0]
                renderer.cam_dir = (GT_POSES[i][1] - GT_POSES[i][0]).normalize()

            renderer.cam_fov = config["FOV_RENDER"]
            ...
            if too_empty or too_close:
                retry_count += 1
                print("Retrying view", i, too_close, too_empty)
            else:
                is_view_valid = True
        ...
        if collect_GT_poses:
            GT_POSES.append((cam_pos, cam_target))

        filename = f"{filename_prefix}shot_{i+1}.png"
        if output_path: 
            renderer.save(str(output_path / filename), silent=True)
            if config.get("SAVE_DEPTH", False) and renderer.generate_depth:
                renderer.save_depth(str(output_path / (filename + "_depth.png")), silent=True)
...
def process_dataset(dataset_name: str, config: Dict[str, Any]) -> None:
    ...
    if "CONFIGS" in config:
        for cfg in config["CONFIGS"]:
            # Merge each config with the general settings
            merged_config = {**config, **cfg}
            configs.append(merged_config)

    for n_views in config["N_VIEWS"]:
        for samples in config["SAMPLES"]:
            configs.append({**config, "N_VIEWS": n_views, "SAMPLES": samples})
    ...
    if DRY_RUN_REPETITIONS > 0:
        ...
        for config in configs:
            n_views = config.get("N_VIEWS", 8)
            samples = config.get("SAMPLES", 8)
            res = config["RESOLUTION"]
            shared_renderer = setup_renderer(config)
            print(f"    - Benchmarking {n_views}p x {samples}s @ {res} ({DRY_RUN_REPETITIONS} repetitions)...")
            repetition_results = []
