...
def sample_inside_unit_sphere(sample, s):
    x = sample[0]
    y = sample[1]
    z = sample[2]
...
if __name__ == "__main__":
    ...
    # settings
    OUT_PATH = os.path.join(ROOT_DIR, 'generated/fb_test/')
    ...
    renderer.denoise_temporal_enabled = True
    renderer.generate_motionvec = True
    renderer.generate_albedo = True
    renderer.generate_depth = True
    renderer.recreate_denoiser()

    renderer.fovea_deg = 10
    renderer.fovea_mult = 2.5
    ...
    for i in range(N_VIEWS):
        ...
        cam_pos_moved_out = (cam_unit_sphere_pos + ((cam_unit_sphere_pos - center).normalize() * radius * (0.1 - variability)))     # Cam position start point from 2.1 times radius down to 1.1 times radius around center
        ...
        variability = min(MAX_VARIABILITY, variability + 0.02)
        ...
        run_data.append(frame_time_ms)

        # store view
        filename = f"view_{i:06}.png"
        renderer.save(os.path.join(OUT_PATH, filename))
        ...
        filename = f"unfov_view_{i:06}.png"
        renderer.save(os.path.join(OUT_PATH, filename))

        #renderer.use_foveation = False
        #renderer.render_multi(SAMPLES)
        #renderer.draw()
        # store view
        #filename = f"GT_view_{i:06}.png"
        #renderer.save_with_alpha(os.path.join(OUT_PATH, filename))


    print(f"Frame times: {run_data}")
    print(f"Median: {statistics.median(run_data)}")
