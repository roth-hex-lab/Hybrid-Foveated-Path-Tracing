...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    if checkpoint:
        ...
        gaussians.restore_nostate(model_params)
        
    gaussians.training_setup(opt)

    
    bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0]
    raw_background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")
    ...
    for iteration in range(first_iter, opt.iterations + 1):   
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    net_image = render_imp(custom_cam, gaussians, pipe, raw_background, scaling_modifer)["render"]
        ...
        if iteration > args.simp_iteration1 and iteration % 50 == 0:
            gaussians.oneupSHdegree()

        if not viewpoint_stack:
            viewpoint_stack = scene.getTrainCameras_warn_up(iteration, args.warn_until_iter, scale=1.0, scale2=2.0).copy()

        if opt.random_background and iteration > opt.densify_until_iter and iteration != args.simp_iteration1 and iteration != args.simp_iteration2:
        ...
        image, viewspace_point_tensor, visibility_filter, radii, accum = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["accumulation"]
        ...
        if viewpoint_cam.gt_alpha_mask is not None:
            ...
            # Add current bg to original image
            gt_image = gt_image * alpha_mask + background.view(3, 1, 1) * (1 - alpha_mask)

            # Add alpha channel
            gt_image = torch.cat((gt_image, alpha_mask), dim=0)
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, raw_background))
            if (iteration in saving_iterations):
                timing = time.time() - time_init
                print(f"Took {timing} for {scene.model_path}")
                with open(os.path.join(scene.model_path, f'timing_{iteration}.txt'), 'w') as speed_file:
                    speed_file.write(str(timing))
            ...
            # # Densification
            if iteration < opt.densify_until_iter and iteration < args.simp_iteration1:
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        test_config = validation_configs[0]
        if test_config["cameras"] is not None and len(test_config["cameras"]) > 0:
            validation_configs = (test_config,)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("  Points : {}".format(scene.gaussians._xyz.shape[0], ".5"))
                print("")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--imp_metric", type=str, default = 'outdoor')
